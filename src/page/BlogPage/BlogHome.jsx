...
import {
  alertMsgForAdminInfo,
  alertMsgForAdminError,
} from "../../utils/alertMsg";
...
const BlogHome = () => {
  ...
  const togglePin = async (articleId) => {
    ...
    try {
      await axios.post(
        `${API_BASE_URL}/posts/${articleId}/pinned`,
        {},
  ...
  const filteredArticles = useMemo(() => {
    return articles
      .filter(
        (article) => filterStatus === "" || article.status === filterStatus
      ) // 只顯示已發布的文章   // 1️⃣ 篩選狀態 (全部 / 已發佈 / 草稿)
      .sort((a, b) => {
    ...
    if (articles.length > 0) {
      ...
      Promise.all(
        articles.map((article) => {
          return axios
            .get(`${API_BASE_URL}/comments/${article.id}`)
            .then((res) => {
  ...
  const handleBannerUpdate = async () => {
    try {
      ...
      if (!imageFile && !imageUrl)
        newErrors.banner = "⚠️ 必須提供圖片（本地或 URL)";
  ...
  useEffect(() => {
    ...
    modalInstanceBannerRef.current = new Modal(modalElement, {
      backdrop: "true",
      Keyboard: true,
    });
  ...
  const openModal = () => {
    if (!modalInstanceBannerRef.current) {
      modalInstanceBannerRef.current = new Modal(modalBannerRef.current, {
        backdrop: "true",
        Keyboard: true,
      });
    }
  ...
  const handleExternalImageEditBlur = async () => {
    if (externalImageEdit) {
      ...
      if (isValid) {
        ...
        setErrors((prev) => ({
          ...prev,
          imageEdit: "⚠️ 圖片 URL 無效，請輸入可預覽的圖片。",
        }));
  ...
  const uploadImageToR2 = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/posts/upload/cover`,
        formData,
        {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (
        titleEdit.trim() !== "" &&
        titleEdit.trim() !== (selectedArticle.title || "").trim()
      ) {
        payload.title = titleEdit.trim();
      }
      if (
        descriptionEdit.trim() !== "" &&
        descriptionEdit.trim() !== (selectedArticle.description || "").trim()
      ) {
        payload.description = descriptionEdit;
      }
      if (finalImageUrl && finalImageUrl !== selectedArticle.image_url) {
        payload.image_url = finalImageUrl;
      }
      if (
        !isQuillContentEmpty(contentEdit) &&
        contentEdit.trim() !== (selectedArticle.content || "").trim()
      ) {
        ...
        if (base64Images.length > 0) {
          try {
            const res = await axios.post(
              `${API_BASE_URL}/posts/upload/content`,
              { files: base64Images },
              {
                headers: {
                  Authorization: `Bearer ${token}`,
      ...
      await axios.patch(
        `${API_BASE_URL}/posts/${selectedArticle.id}`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <>
          ...
          <div className="container">
            <div className="row flex-md-row-reverse">
              <div className="col-xl-3 col-md-4 mb-5">
                <div
                  className="blog-home_header d-flex flex-column align-items-center py-10 px-5 rounded-3 border border-gray_light"
                  style={{ backgroundColor: "#FDFBF5" }}
                >
                  <img
                    className="admin-avatar mb-2 rounded-circle border "
                    src={
                      blogUser?.profile_picture ||
                  ...
                  <ul className="list-unstyled d-flex gap-5 gap-md-3 gap-lg-5 mb-5">
                    <li>
                      <FontAwesomeIcon
                        icon={faEnvelope}
                        size="lg"
                        style={{ color: "#e77605", cursor: "pointer" }}
                      />
                    </li>
                    <li>
                      <FontAwesomeIcon
                  ...
                  <p className="text-gray mt-3 pb-5 border-bottom border-gray">
                    {blogUser.bio}
                  </p>
                        ...
                        return (
                          <div className="accordion-item" key={categoryId}>
                            <h2 className="accordion-header">
                              <button
                                className={`accordion-button text-gray hover-effect ${
                                  isOpen ? "" : "collapsed"
                                }`}
                                type="button"
              ...
              <div className="col-xl-9 col-md-8">
                ...
                >
                  <div className="d-flex flex-column align-items-center align-items-lg-start">
                    <h2
                      className="fs-5 fs-md-3 text-light mb-5"
                      style={{ zIndex: "99" }}
                    >
                      {banner?.title || "歡迎來到 Blog主頁"}
                    </h2>
                    <h4 className="mb-5 text-light" style={{ zIndex: "99" }}>
                      {banner?.subtitle || "請點擊編輯設定你的 Banner"}
                    </h4>
                    ...
                    {isAuthor && (
                      <button
                        type="button"
                        ref={modalTriggerRef}
                        className="btn btn-lg hover-shadow btn-primary btn-click rounded-2"
                        style={{ zIndex: "99" }}
                        onClick={openModal}
                      >
                        {banner ? "編輯" : "建立"}
                ...
                <section className="position-relative mb-5">
                  {swiperArticles.length > 0 ? (
                    <Swiper
                      key={swiperArticles.length} // ✅ Swiper 內容變動時強制刷新
                    ...
                    >
                      <h2 className="fs-5 fs-md-3 text-dark mb-5">
                        請新增文章
                      </h2>
                      <h4 className="text-dark">
                        目前沒有可顯示的文章，請點擊「新增文章」來開始
                      </h4>
                    </div>
                  )}
                </section>
                <div
                  className="blog-home_articleList rounded-3 border border-gray_light py-7 px-8"
                  style={{ backgroundColor: "#FDFBF5", zIndex: "-1" }}
                >
                  <div className="articleList_header">
                    ...
                    {isAuthor && (
                      <div className="d-block d-md-flex justify-content-between align-items-center">
                        <select
                          className="form-select blog-home_articleSelect py-3 mb-6"
                          value={filterStatus}
                        ...
                        <button
                          type="button"
                          className="btn btn-primary btn-click btn-lg mb-5 rounded-2 hover-shadow"
        ...
        <div className="modal-dialog modal-lg">
          <div className="modal-content">
            ...
            <div className="modal-body">
              <label htmlFor="封面圖片" className="form-label fw-medium">
                上傳圖片
              </label>
              <input
                id="封面圖片"
              ...
              <label htmlFor="標題" className="form-label fw-medium">
                Blog主頁標題
              </label>
              <input
                id="標題"
                type="text"
                className="form-control mb-2"
                placeholder="輸入Blog主頁 標題"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
              />
              {errors?.title && <p className="text-danger">{errors?.title}</p>}
              <label htmlFor="副標題" className="form-label fw-medium">
                Blog主頁副標
              </label>
              <input
                id="副標題"
            ...
            <div className="modal-footer">
              ...
              <button
                className="btn btn-secondary btn-click"
                onClick={closeModal}
              >
                關閉
              </button>
          ...
          <div className="modal-content max-h">
            ...
            <div className="modal-body">
              ...
              <input
                ...
                id="輸入封面圖片Url"
                className="form-control mb-2"
                value={externalImageEdit}
                placeholder="輸入封面圖片 URL"
                onChange={handleExternalImageEdit}
                onBlur={handleExternalImageEditBlur}
              />
              {/* 錯誤訊息 */}
              {errors?.imageEdit && (
                <p className="text-danger">{errors?.imageEdit}</p>
              )}
              {imagePreviewEdit && (
                <img
            ...
            <div className="modal-footer">
              ...
              <button
                className="btn btn-sceondary btn-click"
                onClick={closeEditModal}
              >
                關閉
              </button>
      ...
      <ArticlePagination
        ...
        className="hot-article-pagination justify-content-center gap-2 mb-3"
      />
